    ...
    <div>
        <div style="margin-bottom: 5px;text-align: center;">
            ...
            <el-button icon="el-icon-refresh" type="success" style="margin-left: 5px" @click="resetParam()"
                circle></el-button>
            <el-button type="primary" style="margin-left: 5px" @click="add">投放</el-button>
        ...
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" border style="width: 100%" stripe
            ...
            <el-table-column prop="province" label="投放省份" width="150">
            </el-table-column>
            <el-table-column prop="city" label="投放城市" width="150">
            </el-table-column>
            <el-table-column prop="address" label="投放地址">
            </el-table-column>
            <el-table-column prop="count" label="投放数量" width="200">
                <template slot-scope="scope">
                    <el-tag type="primary">{{ scope.row.count }}辆</el-tag> </template>
            </el-table-column>
            <el-table-column prop="createTime" label="投放时间" width="200" :formatter="formatDate">
            </el-table-column>
            ...
            <el-table-column fixed="right" label="操作" prop="operate">
                <template slot-scope="scope">
                    ...
                    <el-button type="success" @click="recovery(scope.row)" size="small" style="margin-left: 5px"
                        v-if="user.role != 2">
                        回收</el-button>
                </template>
            </el-table-column>
        ...
        <el-dialog title="投放表单" :visible.sync="centerDialogVisible" width="800px" center>
        ...
        <el-dialog title="回收表单" :visible.sync="recoveryDialogVisible" width="800px" center>
            <el-form ref="recoveryform" :rules="recoveryrules" :model="recoveryform" label-width="100px">
                <el-form-item label="共享单车" prop="bike">
                    <el-select v-model="recoveryform.bike" placeholder="请选择单车" disabled style="width: 500px;">
                        <el-option v-for="item in bikeData" :key="item.id" :label="item.name" :value="item.id">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="回收数量" prop="count">
                    <el-input-number v-model="recoveryform.count" style="width:500px;" :step="1"
                        :max="invert.count"></el-input-number>辆

                </el-form-item>
                <el-form-item label="回收时间" prop="createTime">
                    <el-date-picker v-model="recoveryform.createTime" :start-placeholder="invert.createTime"
                        style="width: 500px;" value-format="yyyy-MM-dd HH:mm:ss" type="date"
                        placeholder="选择日期"></el-date-picker>
                </el-form-item>
                <el-form-item label="回收地址" prop="address">

                    <el-input disabled style="width: 500px;" v-model="recoveryform.address"
                        placeholder="请选择回收地址"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button @click="recoveryDialogVisible = false">取 消</el-button>
                <el-button type="primary" @click="doRecovery">确 定</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            recoveryrules: {
                bike: [{ required: true, message: "请选择共享单车", trigger: "blur" }],
                count: [{ required: true, message: "请输入回收数量", trigger: "blur" }],
                createTime: [{ required: true, message: "请选择回收时间", trigger: "blur" }],
                address: [{ required: true, message: "请选择回收地址", trigger: "blur" }],
            },
    ...
    methods: {
        recovery(row) {
            this.invert = row;
            this.recoveryform.bike = row.bike;
            this.recoveryform.city = row.city;
            this.recoveryform.lat = row.lat;
            this.recoveryform.lon = row.lon;
            this.recoveryform.province = row.province;
            this.recoveryform.address = row.address
        ...
        doRecovery() {
            this.$refs.recoveryform.validate((valid) => {
                if (valid) {
                    this.$axios
                        .post(this.$httpUrl + "/recovery/save", this.recoveryform)
                        .then((res) => res.data)
                        .then((res) => {
                            console.log(res);
                            if (res.code == 200) {
        ...
        subInvert() {
            this.$axios
                .post(this.$httpUrl + "/invert/sub?id=" + this.invert.id + "&count=" + this.recoveryform.count)
                .then((res) => res.data)
                .then((res) => {
                    console.log(res);
                    if (res.code == 200) {
        ...
        getProvince() {
            this.$axios
                .get(this.$httpUrl + "/invert/provinceList")
                .then((res) => res.data)
                .then((res) => {
        ...
        mod(row) {
            ...
            this.$nextTick(() => {
                //赋值到表单
                this.$axios
                    .get(this.$httpUrl + "/invert/info/" + row.id)
                    .then((res) => {
        ...
        doSave() {
            this.$axios
                .post(this.$httpUrl + "/invert/save", this.form)
                .then((res) => res.data)
                .then((res) => {
                    console.log(res);
                    if (res.code == 200) {
        ...
        doMod() {
            this.$axios
                .post(this.$httpUrl + "/invert/update", this.form)
                .then((res) => res.data)
                .then((res) => {
                    console.log(res);
                    if (res.code == 200) {
        ...
        loadPost() {
            ...
            this.$axios
                .post(this.$httpUrl + "/invert/listPage", {
        ...
        delHandle(id) {
            ...
            console.log(ids);
            this.$confirm("将删除投放一切有关数据， 是否继续?", "提示", {
                ...
                .then(() => {
                    this.$axios
                        .post(this.$httpUrl + "/invert/deleteBatch", ids, {
                            headers: {
